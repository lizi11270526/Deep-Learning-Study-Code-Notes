...
def test_data_process() -> tuple[DataLoader, list]:
    # 加载 FashionMNIST 测试集，并将图像尺寸调整为 224x224，转换为张量
    test_data = FashionMNIST(root=r"F:\Datasets\FashionMNIST", train=False, transform=transforms.Compose([transforms.Resize(size=224), transforms.ToTensor()]), download=True)
    # 获取标签列表
