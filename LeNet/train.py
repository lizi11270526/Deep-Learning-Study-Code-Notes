...
def train_val_data_process() -> tuple[DataLoader, DataLoader]:
    # 加载 FashionMNIST 训练集，并将图像尺寸调整为 28x28，转换为张量
    train_data = FashionMNIST(root=r"F:\Datasets\FashionMNIST", train=True, transform=transforms.Compose([transforms.Resize(size=28), transforms.ToTensor()]), download=True)

    # 按 8:2 比例划分训练集与验证集
    train_data, val_data = random_split(train_data, [round(0.8 * len(train_data)), round(0.2 * len(train_data))])
    # 创建训练集 DataLoader
    train_loader = DataLoader(dataset=train_data, batch_size=64, shuffle=True, num_workers=6)
    # 创建验证集 DataLoade
    val_loader = DataLoader(dataset=val_data, batch_size=64, shuffle=True, num_workers=6)
